...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if self.can_contain(place.ant): # self is contain ant
                self.store_ant(place.ant)
                place.ant = self
            elif place.ant.can_contain(self): # place.ant is contain ant
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # After calling reduce_health, the FireAnt may die. 
        # If that happens, the FireAnt will remove itself from its place, 
        # so we won’t be able to find the bees in that place when we want to deal damage. 
        # Therefore, we need to get the place in advance.
        current_place = self.place

        # all bees in current place reduce health 
        for bee in current_place.bees[:]:
            bee.reduce_health(amount)

        # fireant reduces health
        super().reduce_health(amount)
        
        # if fireant died, all bees in the current place reduce health
        if self.health <= 0:
            for bee in current_place.bees[:]:
...
# The WallAnt class
class WallAnt(Ant):
    "WallAnt can do some protection to our glorious home"
    name = 'Wall'
    food_cost = 4
    implemented = True 
...
# The HungryAnt Class
class HungryAnt(Ant):
    "HungryAnt can eat a bee chewing 3 turns"
    name = 'Hungry'
    food_cost = 4
    chew_cooldown = 3
    implemented = True
    ...
    def action(self, gamestate):
        if self.cooldown <= 0 and self.place.bees:
