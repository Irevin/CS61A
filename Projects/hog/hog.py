...
def boar_brawl(player_score, opponent_score):
    ...
    选择掷零颗骰子的玩家得分是对手得分的十位数字与当前玩家得分的个位数字的绝对差值的三倍,或1,以较大者为准。
    个位数字指的是最右边的数字，十位数字指的是次右边的数字。如果一名玩家的得分是一位数(小于10),则该玩家得分的十位数字为0。
    """
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    opponent_ten_digit = opponent_score // 10 % 10
    player_one_digit = player_score % 10 
    score = max(3 * abs(opponent_ten_digit - player_one_digit), 1)
    return score
    # END PROBLEM 2
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    now_add_score = boar_brawl(score, opponent_score)
    if now_add_score >= threshold:
        return 0
    else:
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    now_score = sus_update(0, score, opponent_score, dice=six_sided)
    if now_score - score >= threshold:
        return 0
    else:
        return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
