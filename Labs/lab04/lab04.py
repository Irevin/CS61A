def divide(quotients, divisors):
    ...
    """
    return {q : [divisor for divisor in divisors if divisor % q == 0] for q in quotients}
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    return sqrt((get_lat(city_a) - get_lat(city_b))**2.0 + (get_lon(city_a) - get_lon(city_b))**2.0)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    distance_a = distance(city_a, city_c)
    distance_b = distance(city_b, city_c)
