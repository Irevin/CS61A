...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    meet_select_paragraph = [paragraph for paragraph in paragraphs if select(paragraph)]
    if len(meet_select_paragraph) <= k:
        return ''
    else:
        return meet_select_paragraph[k]
    # END PROBLEM 1
...
def accuracy(typed, source):
    ...
    100.0
    """
    "I think \t is equals that there is a space so words will be splited normarlly in split function"
    ...
    if len(typed_words) == 0:
        ...
        if typed_words_len > source_words_len:
            errors_number = typed_words_len - source_words_len
        for index in range(min(typed_words_len, source_words_len)):
            if typed_words[index] != source_words[index]:
                errors_number += 1
        return ((typed_words_len - errors_number) / typed_words_len) * 100.0
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    "*** YOUR CODE HERE ***"
    if elapsed <= 60:
        return (60.0 / elapsed) * (len(typed) / 5.0)
    else:
...
def furry_fixes(typed, source, limit):
    ...
    """
    """
    先考虑递归的情况,此递归表面为递归第一个字符+之后的字符,实则用第一个字符做判断，只用递归往后的字符
    把递归的情况考虑明白在加入对limit限制 注意每次用完limit要对limit-1
    """
    # BEGIN PROBLEM 6:
    if limit < 0:
        return limit + 1
...
def time_per_word(words, timestamps_per_player):
    ...
    times = []
    # BEGIN PROBLEM 9
    for p1 in range(len(tpp)):
        player_times = []
        for p2 in range(len(tpp[p1]) - 1):
            player_times.append(tpp[p1][p2 + 1] - tpp[p1][p2])
...
def fastest_words(words_and_times):
    ...
    for word_index in word_indices:
        fastest_player = 0
        fastest_time = get_time(times, fastest_player, word_index)
